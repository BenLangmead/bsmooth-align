...
# BtlBio::Read::Fastq
#
# Routines converting between FASTQ files and BtlBio::Read objects.
#
# Author: Ben Langmead
# Email: langmea@cs.jhu.edu
#

package BtlBio::Read::Fastq;
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(parse_fastq_read);
...
#
sub parse_fastq_pair($$) {
	my ($fh1, $fh2) = @_;
	my ($rd1, $rd2) = (parse_fastq_read($fh1), parse_fastq_read($fh2));
...
sub _test_1() {
	print STDERR "Testing parse_fastq_read 1 ... ";
	my $ostr = "";
	open(my $ofh, '>', \$ostr);
	print {$ofh} qq!
\@Read1
...
!;
	open(my $ifh, '<', \$ostr);
	my $rd1 = parse_fastq_read($ifh);
	my $rd2 = parse_fastq_read($ifh);
	defined($rd2) && die "Expected second call to parse_fastq_read to fail";
...
sub _test_2() {
	print STDERR "Testing parse_fastq_read 2 ... ";
	my $ostr = "";
	open(my $ofh, '>', \$ostr);
	my $qseq = qq!
...
!;
	...
	my $rd1 = parse_fastq_read($ifh);
	my $rd2 = parse_fastq_read($ifh);
	my $rd3 = parse_fastq_read($ifh);
	defined($rd3) && die "Expected third call to parse_fastq_read to fail";
